...
def sample_from_mixture_f(x, pred_weights, pred_means, pred_std, dist):
          ...
          for j, (weights, means, std_devs) in enumerate(
            ...
            if dist.lower() == 'normal':
                ...
                samples[j, 0] = np.random.lognormal(means[index], std_devs[index], size=1)
            elif dist.lower() == 'gamma':
                # Convert mu and var to alpha (shape) and beta (rate)
                mu = means[index]
                var = std_devs[index]  # stddev = sqrt(var), so here it’s actually sigma not var

                # Ensure values are positive and avoid div by zero
                mu = np.clip(mu, 1e-6, 1e6)
                var = np.clip(var**2, 1e-6, 1e6)

                alpha = (mu ** 2) / var
                beta = mu / var  # Note: numpy gamma uses **scale** = 1 / beta

                samples[j, 0] = np.random.gamma(shape=alpha, scale=1.0 / beta, size=1)
            #samples[j, 0] = x[j]
            if j == amount - 1:
              break
          return samples
...
class MDN:
    ...
    def fit(self, X, Y, epochs, batch_size):
        ...
        if n_layer < 1:
          self.layer_last = tf.compat.v1.layers.dense(x, units=self.input_neurons, activation=input_actv)
          self.mu = tf.compat.v1.layers.dense(self.layer_last, units=K, activation=None, name="mu")
          self.var = tf.exp(tf.compat.v1.layers.dense(self.layer_last, units=K, activation=None, name="sigma"))
          self.pi = tf.compat.v1.layers.dense(self.layer_last, units=K, activation=tf.nn.softmax, name="mixing")

        else:
          layer = Dense(units=self.input_neurons, activation=input_actv)(x)

          for i in range(n_layer):
              n_neurons = hidden_neurons[i]
              layer = Dense(units=n_neurons, activation=h_actv)(layer)

          self.layer_last = layer

          self.mu = Dense(units=K, activation=None, name="mu")(self.layer_last)
          self.var = tf.exp(Dense(units=K, activation=None, name="sigma")(self.layer_last))
          self.pi = Dense(units=K, activation=tf.nn.softmax, name="mixing")(self.layer_last)
        ...
        if self.tf_mixture_family == False:
            ...
            if self.dist.lower() == 'normal':
                ...
                epsilon = 1e-6  # to avoid divide-by-zero and log(0)
                safe_mu = tf.clip_by_value(self.mu, epsilon, 1e6)
                safe_var = tf.clip_by_value(self.var, epsilon, 1e6)
                
                alpha = (safe_mu ** 2) / safe_var
                beta = safe_var / safe_mu
